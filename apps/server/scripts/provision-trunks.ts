...
async function main() {
  ...
  try {
    ...
    const match = arr.find((t: any) => (t.numbers || []).includes("+15550101010"));
    if (match) {
  ...
  if (!inboundId) {
    ...
    const inbound = await sip.createSipInboundTrunk("quickvoice-inbound", ["+15550101010"], {});
  ...
  if (!outTwilioId) {
    ...
    const outTwilio = await sip.createSipOutboundTrunk("quickvoice-outbound-twilio", "sip.twilio.com", ["+15550101010"], {
  ...
  try {
    ...
    if (match) {
      ...
      const outTelnyx = await sip.createSipOutboundTrunk("quickvoice-outbound-telnyx", "sip.telnyx.com", ["+15550101010"], {});
